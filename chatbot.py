...
This file contains the code to run the model and act as the server to the
web interface.
"""
...
# Flask
from flask import Flask, request, redirect, Response, current_app, jsonify
...
def _eval_test_set(sess, model, test_buckets):
    ...
    for bucket_id in xrange(len(config.BUCKETS)):
        ...
        _test_info.append('Test bucket {}: loss {}, time {}'.format(bucket_id, step_loss, time.time() - start))
    return _test_info
...
def start_training():
    ...
    with tf.Session() as sess:
        ...
        train_record_file = open(os.path.join(config.PROCESSED_PATH, config.TRAINING_RECORD_FILE), 'a+')
        test_record_file = open(os.path.join(config.PROCESSED_PATH, config.TESTING_RECORD_FILE), 'a+')
        while True:
            try:
                ...
                if iteration % skip_step == 0:
                    _train_info = 'Iter {}: loss {}, time {}'.format(iteration, total_loss/skip_step, time.time() - start)
                    print(_train_info)
...
initial_msg = 'Welcome to TensorChat. Say something. Enter to exit. Max length is ' + str(max_length)
...
def prepare_to_chat():
    ...
    model = Seq2SeqModel(True, batch_size=1)
    model.build_graph()

    saver = tf.train.Saver()

    print("Setting up sess ...")
    sess = tf.Session()
    _check_restore_parameters(sess, saver)
...
def output():
	# serve index page
	return current_app.send_static_file('index.html') #, InitialMsg=initial_msg
